...
setup_cluster() {
    echo_feature "Setting up Raft cluster..."
    
    # Initialize cluster
    echo_info "Initializing cluster on Node 1..."
    local init_response
    if init_response=$(curl -s -X POST "http://127.0.0.1:$NODE1_HTTP/init" 2>/dev/null); then
        echo_success "Cluster initialized: $init_response"
    else
        echo_error "Failed to initialize cluster"
        return 1
    fi
    ...
    echo_info "Adding Node 2 as learner..."
    local add_response
    add_response=$(curl -s -X POST -H "Content-Type: application/json" \
        -d "{\"node_id\":2,\"rpc_addr\":\"127.0.0.1:$NODE2_HTTP\",\"api_addr\":\"127.0.0.1:$NODE2_HTTP\"}" \
        "http://127.0.0.1:$NODE1_HTTP/add-learner" 2>/dev/null)
    echo_success "Node 2 added: $add_response"
    
    sleep 1
    
    # Add Node 3 as learner
    echo_info "Adding Node 3 as learner..."
    add_response=$(curl -s -X POST -H "Content-Type: application/json" \
        -d "{\"node_id\":3,\"rpc_addr\":\"127.0.0.1:$NODE3_HTTP\",\"api_addr\":\"127.0.0.1:$NODE3_HTTP\"}" \
        "http://127.0.0.1:$NODE1_HTTP/add-learner" 2>/dev/null)
    echo_success "Node 3 added: $add_response"
    
    sleep 2
    
    # Change membership to include all nodes
    echo_info "Promoting learners to voting members..."
    ...
    echo_success "Membership changed: $membership_response"
    
    echo_info "Waiting for cluster consensus to stabilize..."
    sleep 5
