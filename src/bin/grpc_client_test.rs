use ferrite::grpc::{
    ...
    SetRequest, GetRequest, HealthRequest, InitializeRequest,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test health check
    let health_response = management_client.health_check(tonic::Request::new(HealthRequest {})).await?;
    println!("Health: {:?}", health_response.get_ref());
    
    // Test initialization
    let init_response = management_client.initialize(tonic::Request::new(InitializeRequest {})).await?;
    ...
    // Test set
    let set_response = kv_client.set(tonic::Request::new(SetRequest {
    ...
    // Test get
    let get_response = kv_client.get(tonic::Request::new(GetRequest {
        key: "test_key".to_string(),
    })).await?;
